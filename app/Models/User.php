...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function accessibleDossiers()
    {
        ...
        if ($this->hasRole('directeur-general')) {
            return $query;
        }
        ...
        if ($this->hasRole('manager')) {
            ...
            return $query->where(function ($q) use ($subordinateIds) {
                $q->where('created_by', $this->id)
                ->orWhereHas('collaborateurs', fn($s) =>
                    $s->where('collaborateur_dossier.user_id', $this->id)
                        ->where('collaborateur_dossier.is_active', true)
                );

                if (!empty($subordinateIds)) {
                    $q->orWhereIn('created_by', $subordinateIds)
                    ->orWhereHas('collaborateurs', fn($s) =>
                        $s->whereIn('collaborateur_dossier.user_id', $subordinateIds)
                            ->where('collaborateur_dossier.is_active', true)
                    );
                }
            });
